...
// IsEmpty checks whether the provided AuthokClientInfo data is nil or has no data to allow
// short-circuiting the "Authok-Client" header configuration.
...
func AuthokClientInfoTransport(base http.RoundTripper, authokClientInfo *AuthokClientInfo) (http.RoundTripper, error) {
	...
	authokClientJSON, err := json.Marshal(authokClientInfo)
	if err != nil {
		return nil, err
	}

	authokClientEncoded := base64.StdEncoding.EncodeToString(authokClientJSON)

	return RoundTripFunc(func(req *http.Request) (*http.Response, error) {
		req.Header.Set("Authok-Client", authokClientEncoded)
		return base.RoundTrip(req)
	}), nil
}
...
func WithAuthokClientInfo(authokClientInfo *AuthokClientInfo) Option {
	return func(c *http.Client) {
		...
		transport, err := AuthokClientInfoTransport(c.Transport, authokClientInfo)
		if err != nil {
			return
		}
