...
func TestCustomDomainManager_Create(t *testing.T) {
	...
	expected := &CustomDomain{
		Domain:    authok.Stringf("%d.auth.uat.authok.com", time.Now().UTC().Unix()),
		Type:      authok.String("authok_managed_certs"),
		TLSPolicy: authok.String("recommended"),
	}
...
func TestCustomDomainManager_Update(t *testing.T) {
	...
	err := api.CustomDomain.Update(customDomain.GetID(), &CustomDomain{TLSPolicy: authok.String("recommended")})
	assertNoCustomDomainErr(t, err)
