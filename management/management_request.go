...
func (m *Management) NewRequest(
	...
	if payload != nil && strings.ToLower(method) != "get" {
...
// RequestOption configures a call (typically to retrieve a resource) to Authok with
// query parameters.
...
func PerPage(items int) RequestOption {
	return newRequestOption(func(r *http.Request) {
		q := r.URL.Query()
		q.Set("page_size", strconv.FormatInt(int64(items), 10))
		r.URL.RawQuery = q.Encode()
	})
}
...
//
// See: https://authok.com/docs/users/search/v3/query-syntax
...
// Header configures a request to add HTTP headers to requests made to Authok.
