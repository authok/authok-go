...
func TestPrompt(t *testing.T) {
	...
	t.Cleanup(func() {
		err := api.Prompt.Update(&Prompt{
			...
			IdentifierFirst:             authok.Bool(false),
			WebAuthnPlatformFirstFactor: authok.Bool(false),
		})
	...
	t.Run("Update to the new identifier first experience with biometrics", func(t *testing.T) {
		err := api.Prompt.Update(&Prompt{
			...
			IdentifierFirst:             authok.Bool(true),
			WebAuthnPlatformFirstFactor: authok.Bool(true),
		})
