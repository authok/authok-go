...
func TestClient_Create(t *testing.T) {
	...
	expectedClient := &Client{
		Name:           authok.Stringf("Test Client (%s)", time.Now().Format(time.StampMilli)),
		Description:    authok.String("This is just a test client."),
		AppType:        authok.String("web"),
...
func TestJWTConfiguration(t *testing.T) {
	t.Run("MarshalJSON", func(t *testing.T) {
		for clientJWTConfiguration, expected := range map[*ClientJWTConfiguration]string{
			{}:                                    `{}`,
			{LifetimeInSeconds: authok.Int(1000)}: `{"lifetime_in_seconds":1000}`,
		} {
	...
	t.Run("UnmarshalJSON", func(t *testing.T) {
		for jsonBody, expected := range map[string]*ClientJWTConfiguration{
			...
			`{"lifetime_in_seconds":1000}`:   {LifetimeInSeconds: authok.Int(1000)},
			`{"lifetime_in_seconds":"1000"}`: {LifetimeInSeconds: authok.Int(1000)},
		} {
...
func givenAClient(t *testing.T) *Client {
	...
	client := &Client{
		Name:              authok.Stringf("Test Client (%s)", time.Now().Format(time.StampMilli)),
		Description:       authok.String("This is just a test client."),
		OrganizationUsage: authok.String("allow"),
		AppType:           authok.String("web"),
	}
