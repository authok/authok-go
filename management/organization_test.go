...
func TestOrganizationManager_Create(t *testing.T) {
	...
	org := &Organization{
		Name:        authok.String(fmt.Sprintf("test-organization%v", rand.Intn(999))),
		DisplayName: authok.String("Test Organization"),
		Branding: &OrganizationBranding{
			LogoURL: authok.String("https://example.com/logo.gif"),
		},
	}
...
func TestOrganizationManager_Update(t *testing.T) {
	...
	err := api.Organization.Update(org.GetID(), &Organization{Name: authok.String("new-org-name")})
	assert.NoError(t, err)
...
func TestOrganizationManager_AddConnection(t *testing.T) {
	...
	conn := givenAConnection(t, connectionTestCase{connection: Connection{
		Name:        authok.String(fmt.Sprintf("test-conn%v", rand.Intn(999))),
		DisplayName: authok.String(fmt.Sprintf("Test Connection %v", rand.Intn(999))),
		Strategy:    authok.String(ConnectionStrategyAuthok),
		EnabledClients: &[]string{
			os.Getenv("AUTHOK_CLIENT_ID"),
			client.GetClientID(),
		},
	}})
	orgConn := &OrganizationConnection{
		ConnectionID:            conn.ID,
		AssignMembershipOnLogin: authok.Bool(true),
	}
...
func TestOrganizationManager_UpdateConnection(t *testing.T) {
	...
	err := api.Organization.UpdateConnection(
		...
		&OrganizationConnection{
			AssignMembershipOnLogin: authok.Bool(false),
		},
	)
	assert.NoError(t, err)
...
func TestOrganizationManager_CreateInvitation(t *testing.T) {
	...
	orgInvite := &OrganizationInvitation{
		Inviter: &OrganizationInvitationInviter{
			Name: authok.String("Test Inviter"),
		},
		Invitee: &OrganizationInvitationInvitee{
			Email: authok.String("test@example.com"),
		},
...
func TestOrganizationManager_Members(t *testing.T) {
	...
	orgList, err := api.User.Organizations(user.GetID())
	assert.NoError(t, err)
	assert.Len(t, orgList.Organizations, 1)
}
