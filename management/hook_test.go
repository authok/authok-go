...
func TestHookManager_Create(t *testing.T) {
	...
	hook := &Hook{
		Name:      authok.String("testing-hook-creation"),
		Script:    authok.String("function (user, context, callback) { callback(null, { user }); }"),
		TriggerID: authok.String("pre-user-registration"),
...
func TestHookManager_Update(t *testing.T) {
	...
	updatedHook := &Hook{
		Script:  authok.String("function (user, context, callback) { console.log('hooked!'); callback(null, { user }); }"),
...
func givenAHook(t *testing.T, secrets HookSecrets) *Hook {
	...
	hook := &Hook{
		Name:      authok.String(fmt.Sprintf("test-hook%d", rand.Intn(999))),
		Script:    authok.String("function (user, context, callback) { callback(null, { user }); }"),
		TriggerID: authok.String("pre-user-registration"),
		Enabled:   authok.Bool(false),
	}
